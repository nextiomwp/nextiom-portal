...
export default function LinkPreviewCard({ url, isOnBrand, c }) {
  ...
  if (!preview) {
    return (
      <div style={{ marginTop: 6, padding: '8px 12px', borderRadius: 8, background: isOnBrand ? 'rgba(255,255,255,0.1)' : c.hover, display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: isOnBrand ? 'rgba(255,255,255,0.6)' : c.subText }}>
        ...
        <div style={{ width: 12, height: 12, borderRadius: '50%', border: `2px solid ${c.subText}`, borderTopColor: 'transparent', animation: 'lpspin 0.6s linear infinite' }} />
        Loading preview…
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        {preview.image && (
          <div style={{ width: 80, minHeight: 60, flexShrink: 0, background: c.hover, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
            <img
              src={preview.image}
              alt=""
        ...
        <div style={{ padding: '8px 10px', flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
          {preview.title && (
            <div style={{ fontSize: 12, fontWeight: 600, color: isOnBrand ? '#fff' : c.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {preview.title}
            </div>
          )}
          {preview.description && (
            <div style={{ fontSize: 11, color: isOnBrand ? 'rgba(255,255,255,0.7)' : c.subText, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {preview.description}
            </div>
          )}
          <div style={{ fontSize: 10, color: isOnBrand ? 'rgba(255,255,255,0.5)' : '#60a5fa', display: 'flex', alignItems: 'center', gap: 3 }}>
