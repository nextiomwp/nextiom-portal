...
import { ArrowLeft, Save, Mail, Trash2, Shield, Eye, EyeOff, Server, HardDrive, Database, Wifi, Activity, FileText } from 'lucide-react';
...
function toDbUpdates(updates) {
  ...
  if (Object.prototype.hasOwnProperty.call(dbUpdates, 'packageName')) {
    dbUpdates.package_name = dbUpdates.packageName;
    delete dbUpdates.packageName;
  }
  if (Object.prototype.hasOwnProperty.call(dbUpdates, 'expiryDate')) {
    dbUpdates.expiry_date = dbUpdates.expiryDate;
    delete dbUpdates.expiryDate;
  }
  if (Object.prototype.hasOwnProperty.call(dbUpdates, 'autoRenew')) {
    dbUpdates.auto_renew = dbUpdates.autoRenew;
    delete dbUpdates.autoRenew;
  }
...
function AdminHostingDetailsView({ pkg, customer, onBack }) {
  ...
  const [details, setDetails] = useState(normalizeDetails(pkg));
  ...
  useEffect(() => {
    setDetails(normalizeDetails(pkg));
  }, [pkg]);
  ...
  useEffect(() => {
    const fetchRelated = async () => {
      ...
      const [hostingReqs, allDomains] = await Promise.all([
        getHostingRequests(),
        getDomains()
      ]);

      const filteredReqs = (hostingReqs || []).filter((r) => {
        if (r.package_id) return r.package_id === pkg.id;
        return r.customer_id === customer?.id && r.package_name === (pkg.package_name || pkg.packageName);
      });

      const filteredDomains = (allDomains || []).filter((d) => d.customer_id === customer?.id);
      setRequests(filteredReqs);
      setDomains(filteredDomains);
    };

    fetchRelated();
  }, [pkg.id, pkg.package_name, pkg.packageName, customer?.id]);
  ...
  const handleUpdate = async (section, data) => {
    ...
    try {
      ...
      if (section === 'usage') updates = { usage: { ...details.usage, ...data } };
      if (section === 'cpanel') updates = { cpanel: { ...details.cpanel, ...data } };
      if (section === 'notes') updates = { notes: data };

      const dbUpdates = toDbUpdates(updates);
      await updateHostingPackage(pkg.id, dbUpdates);

      setDetails((prev) => ({ ...prev, ...updates }));
      setLogs(getHostingActivityLog(pkg.id));
      toast({ title: 'Updated Successfully', description: 'Package details have been saved.' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => toast({ title: 'Invoice Sent', description: `Invoice sent to ${customer?.email || 'customer'}` })}>
            <FileText className="w-4 h-4 mr-2" /> Send Invoice
          </Button>
          <Button variant="outline" onClick={() => toast({ title: 'Email Sent', description: `Notification sent to ${customer?.email || 'customer'}` })}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <input
                  type="date"
                  value={details.expiryDate ? String(details.expiryDate).split('T')[0] : ''}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) => setDetails({ ...details, usage: { ...details.usage, emailAccounts: parseInt(e.target.value || '0', 10) } })}
                  className="w-full p-2 border border-slate-300 rounded-md"
                />
              </div>
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) => setDetails({ ...details, usage: { ...details.usage, databases: parseInt(e.target.value || '0', 10) } })}
                  className="w-full p-2 border border-slate-300 rounded-md"
                />
              </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <h3 className="text-sm font-bold text-slate-800 mb-3">Related Requests</h3>
            <div className="space-y-2 max-h-[220px] overflow-y-auto pr-2">
              {requests.length > 0 ? requests.map((req) => (
                <div key={req.id} className="p-2 border border-slate-200 rounded-md">
                  <p className="text-xs font-medium text-slate-700">{req.package_name || 'Hosting Request'}</p>
                  <p className="text-[11px] text-slate-500">{req.status || 'pending'} • {req.created_at ? format(new Date(req.created_at), 'PP') : '-'}</p>
                </div>
              )) : (
                <p className="text-xs text-slate-500 italic">No related requests.</p>
              )}
            </div>
          </div>
        </div>
