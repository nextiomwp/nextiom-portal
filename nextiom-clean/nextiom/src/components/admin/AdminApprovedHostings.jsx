...
function parsePackageType(raw) {
  ...
  const hostingType = dashIdx >= 0 ? mainPart.slice(0, dashIdx).trim() : mainPart;
  ...
  return { hostingType, planName, billing };
}

function AdminApprovedHostings({ isDark = true }) {
  ...
  const filteredHostings = hostings.filter(h => {
    ...
    const matchSearch =
      (parsed.hostingType + ' ' + parsed.planName + ' ' + (h.customers?.name || '') + ' ' + (h.package_type || ''))
  ...
  const handleDelete = async (h) => {
    ...
    try {
      ...
      toast({ title: 'Error', description: 'Failed to delete hosting', variant: 'destructive' });
    }
  };
  ...
  const handleNotify = async (h) => {
    ...
    const packageName = `${parsed.hostingType} — ${parsed.planName}`;
    try {
      await addNotification({
        customer_id: h.customer_id,
        type: 'hosting_expiry',
        title: `Hosting Expiry Notice — ${packageName}`,
        message: `Dear valued customer,\n\nThis is a courtesy reminder that your hosting package (${packageName}) is scheduled to expire in ${daysText} (on ${expiryStr}).\n\nTo ensure continued availability of your website and associated services, we strongly recommend renewing your hosting plan before the expiry date. Service interruptions may occur if the package is not renewed in time.\n\nFor assistance with renewal or to explore upgrade options, please reach out to our support team.\n\nBest regards,\nNEXTIOM Team`,
      });
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: '0 0 280px' }}>
          ...
          <input
            ...
            placeholder="Search hosting type, plan or customer..."
            value={searchTerm}
      ...
      <div style={cardS}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            ...
            {filteredHostings.length === 0 && <tr><td colSpan={6} style={emptyS}>No approved hostings found</td></tr>}
          </tbody>
        </table>
      </div>
...
export default AdminApprovedHostings;
