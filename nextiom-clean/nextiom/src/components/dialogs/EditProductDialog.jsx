...
import { updateProduct, addNotification } from '@/lib/storage';
...
export default function EditProductDialog({ open, onOpenChange, product, onSuccess, isDark, c }) {
  ...
  useEffect(() => {
    if (product && open) {
      ...
      const duration = product.license_type || 'yearly';
      const hasRenewal = !!product.renewal_enabled;
      let renewalPercentage = '';
      if (hasRenewal && product.price > 0 && product.renewal_price != null) {
        const pct = ((product.renewal_price - product.price) / product.price) * 100;
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const isDigital = customForm.category === 'digital';
      const updates = {
        ...
        download_url: isDigital ? customForm.downloadUrl.trim() : null,
        category: customForm.category,
        image_url: null,
        license_key: customForm.licenseKey.trim() || null,
        license_type: isDigital ? (customForm.accessMethod === 'one_time' ? null : customForm.duration) : null,
        license_registration: isDigital ? (customForm.accessMethod === 'license_auto') : false,
        automatic_updates: isDigital ? (customForm.accessMethod === 'license_auto') : false,
        manual_updates: isDigital ? (customForm.accessMethod === 'manual_no_license') : false,
      ...
      addNotification({ customer_id: null, type: 'product_updated', title: `Product Updated — ${updates.name}`, message: `Admin updated product details for "${updates.name}".` }).catch(() => {});
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            <div style={{ fontWeight: 700, fontSize: 16, color: text }}>Edit Product</div>
          </div>
          <button
