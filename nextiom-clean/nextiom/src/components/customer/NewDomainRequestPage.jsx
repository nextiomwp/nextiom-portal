...
function NewDomainRequestPage({ onSuccess, user, isDark = false, c = {} }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (domainError) throw new Error(domainError?.message || 'Failed to create domain request');
      if (!domainRequest) throw new Error('No data returned from domain request insert');
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '64px 0', textAlign: 'center' }}>
        ...
        <p style={{ color: subText, fontSize: 14, maxWidth: 400, marginBottom: 24, lineHeight: 1.6 }}>
          Your request for <strong style={{ color: text }}>{domainName}{extension}</strong> has been received. You will be notified once admin reviews it.
        </p>
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div>
          <h1 style={{ color: text, fontSize: 20, fontWeight: 800 }}>Register New Domain</h1>
      ...
      <form onSubmit={handleSubmit} style={{ ...sectionCard, display: 'flex', flexDirection: 'column', gap: 18 }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <div style={{ position: 'relative', flex: 1 }}>
              <Globe style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', width: 16, height: 16, color: subText }} />
              <input
                type="text"
                value={domainName}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', border: `1px solid ${borderStrong}`, borderRadius: 10, background: panel2 }}>
          <input type="checkbox" id="domain_auto_renew" checked={autoRenew} onChange={e => setAutoRenew(e.target.checked)} style={{ width: 16, height: 16, accentColor: brand }} />
          <label htmlFor="domain_auto_renew" style={{ fontSize: 13, color: text, cursor: 'pointer' }}>Enable Auto Renewal</label>
        </div>
