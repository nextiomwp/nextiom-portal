...
export const getLicenses = async (customerId) => {
  let query = supabase.from('licenses').select('*, products(id, name, price, currency, type, description, download_url, license_type, license_registration, manual_updates, automatic_updates, category, image_url, renewal_enabled, renewal_price, renewal_date, renewal_period_days, license_key, version, note, documentation)');
...
export const createTicket = async (customerId, subject, priority = 'normal', department = 'Technical Support', selectedQuickAction = null, createdByAdmin = false) => {
  await assertPortalActionsAllowed();
  const { data, error } = await supabase
    .from('tickets')
    .insert([{ customer_id: customerId, subject, priority, status: 'open', department, selected_quick_action: selectedQuickAction, created_by_admin: createdByAdmin }])
    .select()
    .single();
